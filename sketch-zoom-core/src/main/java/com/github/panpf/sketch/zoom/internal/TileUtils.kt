...
import com.github.panpf.decode.internal.maxBitmapSize
...
internal fun findSampleSize(
    ...
    require(canSubsampling(imageSize = imageSize, sampledSize = sampledSize)) {
        "imageSize $imageSize and sampledSize $sampledSize must have the same aspect ratio"
    }

    val scaledWidthRatio = (imageSize.width / (sampledSize.width * scale))
    var sampleSize = 1
...
internal fun canSubsampling(imageSize: Size, sampledSize: Size): Boolean {
    val imageRatio = (imageSize.width / imageSize.height.toFloat()).format(1)
    val sampledRatio = (sampledSize.width / sampledSize.height.toFloat()).format(1)
    return abs(imageRatio - sampledRatio).format(1) <= 0.1f
}